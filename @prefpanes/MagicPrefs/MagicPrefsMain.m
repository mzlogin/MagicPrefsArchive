...
// 返回 SF Symbol 图标（macOS 11+），旧系统回退到 bundle 内的 PNG
- (NSImage *)sfIcon:(NSString *)symbolName size:(CGFloat)pts fallback:(NSString *)fallbackName {
    if (@available(macOS 11.0, *)) {
        NSImageSymbolConfiguration *cfg = [NSImageSymbolConfiguration configurationWithPointSize:pts weight:NSFontWeightRegular scale:NSImageSymbolScaleSmall];
    ...
    return [[[NSImage alloc] initWithContentsOfFile:[[self bundle] pathForImageResource:fallbackName]] autorelease];
}

- (NSImage *)icoImage:(NSString *)name {
    if ([name isEqualToString:@"ico_app"])     return [self sfIcon:@"app.fill"                    size:14 fallback:name];
    if ([name isEqualToString:@"ico_key"])     return [self sfIcon:@"keyboard"                    size:14 fallback:name];
    if ([name isEqualToString:@"ico_script"])  return [self sfIcon:@"applescript.fill"            size:14 fallback:name];
    if ([name isEqualToString:@"ico_custom"])  return [self sfIcon:@"star.fill"                   size:14 fallback:name];
    if ([name isEqualToString:@"ico_windows"]) return [self sfIcon:@"rectangle.3.group.fill"      size:14 fallback:name];
    if ([name isEqualToString:@"ico_desktop"]) return [self sfIcon:@"menubar.rectangle"           size:14 fallback:name];
    if ([name isEqualToString:@"ico_pointer"]) return [self sfIcon:@"cursorarrow"                 size:14 fallback:name];
    if ([name isEqualToString:@"ico_window"])  return [self sfIcon:@"macwindow"                   size:14 fallback:name];
    // fallback: load from bundle
    return [[[NSImage alloc] initWithContentsOfFile:[[self bundle] pathForImageResource:name]] autorelease];
}
...
-(void)addPop:(id)sender{
    ...
    NSImage *appImg = [self icoImage:@"ico_app"];
    NSImage *keyImg = [self icoImage:@"ico_key"];
    NSImage *scriptImg = [self icoImage:@"ico_script"];
    ...
    [[targetMenu itemWithTitle:@"Custom Actions"] setImage:[self icoImage:@"ico_custom"]];
    [[targetMenu itemWithTitle:@"Application Windows"] setImage:[self icoImage:@"ico_windows"]];
    [[targetMenu itemWithTitle:@"Desktop"] setImage:[self icoImage:@"ico_desktop"]];
    [[targetMenu itemWithTitle:@"Toggle Mouse Finger Cursor"] setImage:[self icoImage:@"ico_pointer"]];
    [[targetMenu itemWithTitle:@"Toggle Trackpad Finger Cursor"] setImage:[self icoImage:@"ico_pointer"]];
...
-(void)customKeyPane:(id)sender{
    [customImage setImage:[self icoImage:@"ico_key"]];
...
-(void)customScriptPane:(id)sender{
    [customImage setImage:[self icoImage:@"ico_script"]];
...
-(void)customAppPane:(id)sender{
    [customImage setImage:[self icoImage:@"ico_app"]];
...
-(IBAction)presetAppPane:(id)sender{
    ...
    [customImage setImage:[self icoImage:@"ico_window"]];
